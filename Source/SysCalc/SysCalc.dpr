...
uses
  ...
  mSysCalc in 'mSysCalc.pas' {dmSysCalc: TDataModule},
  CadTMot in 'CadTMot.pas' {CdTipoMotores},
  CadParam in 'CadParam.pas' {CdParametros},
  CadCalc in 'CadCalc.pas' {CdCalculos},
...
var
  a: Integer;
const
  NomForms: array [1..3] of string =
    ('CdTipoMotores', 'CdParametros', 'CdCalculos');
...
begin
  ...
  case IndexOfCnst of
    01: Result := TCdTipoMotores.Create(Application);
    02: Result := TCdParametros.Create(Application);
    03: Result := TCdCalculos.Create(Application);
  end;
end;
...
begin
  ...
  try
    ...
    Application.CreateForm(TdmSysCalc, dmSysCalc);
    Dados.Image32.GetIcon(0, Application.Icon);
    with Dados do
    begin
      Dados.ConfigFiles;
      dmSysCalc.ConfigFiles;
    ...
    if Assigned(dmSysCalc) then dmSysCalc.Free;

...
uses FuncoesDB, DateUtils, mSysBcCx, ArqSqlBcCx, Dado, ProcType, GridRow, DB;
...
{  ShowMessage('LoadGrids: Data Inicial: ' + DateToStr(FStartBaseDate) + #10#13 +
              'LoadGrids: Data Final: ' + DateToStr(FEndBaseDate));}
  ReleaseTreeNodes(Sender);
  with dmSysBcCx do
  begin
    if qrFinance.Active then qrFinance.Close;
    qrFinance.SQL.Clear;
    qrFinance.SQL.Add(SqlLancaAllCta);
    ...
    try
      ...
      if (qrFinance.Params.FindParam('FlagTCta') <> nil) then
         if (ATypeCount = atOther) then
           qrFinance.ParamByName('FlagTCta').AsInteger := Integer(ATypeCount)
         else
           qrFinance.ParamByName('FlagTCta').AsInteger := 0;
